...
export default function Modal(props) {
  ...
  const [formData, setFormData] = useState({
    ...
    location: '',
    date: '',
	lectureName:props.data.name,
	lectureEmail:props.data.email,
	lecturePhone:props.data.phone,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const response = await fetch('https://hook.eu1.make.com/lm4shyky6f1oicslb745cccoleqe8v9l', {
          method: 'POST',
          headers: {
      ...
      <dialog id="my_modal_2" className="modal">
        <div className="modal-box flex flex-col justify-center items-center">
          ...
          <div className="modal-action flex flex-col justify-center items-center">
            <form className="flex-row gap-4 justify-center items-center" onSubmit={handleSubmit}>
              ...
              <label className="input input-bordered flex items-center gap-2 mt-2">
                <input
                  ...
                  onChange={handleChange}
                  className={`grow ${formErrors.participants ? 'input-error' : ''}`}
                  placeholder="מס משתתפים"
                />
              </label>
              {formErrors.participants && <p className="text-red-500 text-sm">{formErrors.participants}</p>}
              <label className="input input-bordered flex items-center gap-2 mt-2">
                <input
                  type="text"
                  name="location"
